...
export default function StickyHeadTable() {
  ...
  return (
    <MainCard content={false} title="Sticky Header 1" secondary={<CSVExport data={rows} filename={'sticky-table-data.csv'} />}>
      {/* table */}
      <TableContainer sx={{ maxHeight: 430 }}>
        <Table stickyHeader aria-label="sticky table">
          <TableHead
            sx={(theme) => ({
              '& th': { borderTop: '1px solid', borderBottom: `2px solid ${theme.palette.divider} !important`, borderColor: 'divider' }
            })}
          >
            <TableRow>
              {columns.map((column) => (
